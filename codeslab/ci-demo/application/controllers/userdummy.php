 ...
 */
class Userdummy extends CI_Controller {
	...
	public function register() {
		...
		$query = $this->db->query(
			"INSERT INTO user (FIRST_NAME, LAST_NAME, PHONE, USERNAME, PASSWORD, TYPE, STATUS_ID) VALUES (?, ?, ?, ?, ?, ?, ?)",
			array(
			      $postValues['FIRST_NAME'], $postValues['LAST_NAME'], $postValues['PHONE'], $postValues['USERNAME'],
			      $encryptString, $postValues['TYPE'], $postValues['STATUS_ID']
			      )
		);
		...
		$this->output->set_output(json_encode(array('mailbody' => $emailContent, 'message' => "Congratulation! Registration is completed successfully!")));
	}
